...
import { resultAtom } from "@/state/transcribe-atoms";
import {
  currentIndexAtom,
  revealedAtom,
  scoreAtom,
...
export default function QuizPage() {
  ...
  const normalizedQuestions = normalizeQuestions(result?.quiz?.questions ?? []);

  const totalQuestions =
    normalizedQuestions.length || result?.quiz?.meta?.total_questions || result?.quizCount || 0;
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      const params = new URLSearchParams({ callbackUrl: "/quiz" });
  ...
  const videoMinutes = result?.durationSeconds ? Math.round(result.durationSeconds / 60) : null;
  ...
  if (!result?.quiz || normalizedQuestions.length === 0) {
    return (
        ...
        <Card className="max-w-lg border-white/10 bg-white/10 text-white">
          <CardHeader>
            ...
            <CardDescription className="text-white/75">
              Silakan proses transkrip YouTube terlebih dahulu, lalu tombol &quot;Mulai kuis&quot; akan muncul di
              halaman transcribe.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap gap-3">
            <Button asChild>
              <Link href="/transcribe">Kembali ke transcribe</Link>
            </Button>
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-gradient-to-br from-[#120b34] via-[#0f0a26] to-[#0a0b1a] pb-16 text-white">
      ...
      <div className="container relative max-w-5xl space-y-6 py-14">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-2">
            <p className="text-sm font-semibold text-emerald-200">Quiz kajian</p>
            <h1 className="text-3xl font-bold sm:text-4xl">
              {result.summary?.short ?? "Latihan soal dari transkrip YouTube"}
            </h1>
            <p className="text-white/75">
              {totalQuestions} soal pilihan ganda · Durasi video {videoMinutes ? `${videoMinutes} menit` : "?"} · Sumber
              : {result.youtubeUrl ?? "URL tidak tersedia"}
            </p>
          </div>
              ...
              {revealed[currentIndex] ? (
                <div className="space-y-2 rounded-xl border border-white/10 bg-white/5 p-3 text-sm text-white/80">
                  <p className="flex items-center gap-2 font-semibold text-white">
                    <Sparkles className="h-4 w-4 text-emerald-300" /> Pembahasan
                  </p>
          ...
          <div className="space-y-4">
            <Card className="border-white/10 bg-white/10 text-white">
              ...
              <CardContent className="space-y-3">
                ...
                {!allAnswered ? (
                  <p className="text-xs text-amber-200">Jawab semua soal untuk melihat skor akhir.</p>
                ) : null}
              </CardContent>
            </Card>
              ...
              <Card className="border-white/10 bg-emerald-600/15 text-white shadow-lg">
                ...
                <CardContent className="space-y-3">
                  <div className="rounded-xl border border-white/10 bg-white/10 p-3 text-sm text-white/80">
                    ...
                    <p>
                      Pertajam pemahaman dengan mengulang soal yang salah atau kembali ke ringkasan untuk membaca ulang
                      konteks.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button asChild className="bg-white text-[#120b34]">
                      <Link href="/transcribe">Buka ringkasan lagi</Link>
                    </Button>
                    <Button
                      variant="ghost"
                      className="text-white/80 hover:bg-white/10"
